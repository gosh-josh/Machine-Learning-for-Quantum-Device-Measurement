...
class double_dot_2d:
    ...
    def __init__(self, name, block_size , allowed_n_blocks , gates , gate_numbers , default_gates, initial_gate_cu_cv_num, pygor_mode = 'none', pygor_xmlip = None,starting_position = None, random_pixel=True):

        self.random_pixel = random_pixel
        self.default_gates = default_gates
        self.gates = gates
        self.gate_numbers = gate_numbers
        self.initial_control_voltages = [default_gates[initial_gate_cu_cv_num[0]], default_gates[initial_gate_cu_cv_num[1]]]
        self.control_gates = [gates[initial_gate_cu_cv_num[0]], gates[initial_gate_cu_cv_num[1]]]

        # D is the dimension of each patch
        # self.dim is the dimension of blocks in the images
        self.block_size = block_size
        self.allowed_n_blocks = allowed_n_blocks
        self.D = 9
        self.K = 6
        ...
        #self.sweep_parameters_to_start()
        self.pygor.setvals(self.gates, self.default_gates)
        ...
        if starting_position == None:
            self.starting_pos = [math.floor(self.allowed_n_blocks / 2.0), math.floor(self.allowed_n_blocks / 2.0)]
        ...
        self.block_centre_voltages = [0] * self.allowed_n_blocks
        ...
        for ii in range(self.allowed_n_blocks):
            ...
            self.block_centre_voltages[ii] = [0] * self.allowed_n_blocks

        for  i in range(self.allowed_n_blocks):
            for j in range(self.allowed_n_blocks):
                self.block_centre_voltages[i][j] = [self.initial_control_voltages[0] - self.block_size * (i - math.floor(self.allowed_n_blocks/2.0)),self.initial_control_voltages[1] - self.block_size * (j- math.floor(self.allowed_n_blocks/2.0))]
        ...
        measurement, statistics = self.primary_measurement([self.starting_pos[0], self.starting_pos[1]])
    ...
    def sweep_parameters_to_start(self):
        print("Sweeping")
        
        all_current_parameters = self.pygor.get_params()
        current_parameters = []
        #print('current parameters',all_current_parameters)
        
        for index, number in enumerate(self.gate_numbers):
            #current_parameters[index] = all_current_parameters[number]
            current_parameters.append(all_current_parameters[number])

        print('current parameters',current_parameters)
        for index, parameter in enumerate(current_parameters):
            print(index)
            print(parameter)
            mv = int(abs(parameter - self.default_gates[index]))
            self.pygor.do1d(self.gates[index],parameter,self.default_gates[index],mv)
        
        print('Parameters after sweep',self.pygor.get_params())

    def primary_measurement(self,location):
        if self.random_pixel == True:
            return self.random_point_measurement(location)
        else:
            return self.take_measurement(location)

    def pre_classify(self):

        print('Parameters after sweep',self.pygor.get_params())
        
        centre_voltages = self.block_centre_voltages[0][0]

        self.pygor.setvals([self.control_gates[0],self.control_gates[1]],[centre_voltages[0],centre_voltages[1]])
        self.trace_0 = self.pygor.do1d(self.control_gates[0],centre_voltages[0], centre_voltages[0] - self.block_size*self.allowed_n_blocks, 100)
        self.pygor.setval(self.control_gates[0],centre_voltages[0])
        self.trace_1 = self.pygor.do1d(self.control_gates[1],centre_voltages[1], centre_voltages[1] - self.block_size*self.allowed_n_blocks, 100)
        self.pygor.setvals([self.control_gates[0], self.control_gates[1]], [self.initial_control_voltages[0], self.initial_control_voltages[1]])

        plt.plot(self.trace_0.data[0])
        plt.title("Trace 0")
        plt.show()
        
        plt.plot(self.trace_1.data[0])
        plt.title("Trace 1")
        plt.show()

        self.environment_max_current = max([max(self.trace_0.data[0]),max(self.trace_1.data[0])])
        self.environment_min_current = min([min(self.trace_0.data[0]),min(self.trace_1.data[0])])

        self.standard_deviation_trace_0 = np.std(self.trace_0.data[0])
        self.standard_deviation_trace_1 = np.std(self.trace_1.data[0])
        ...
        self.threshold_upper = self.trace_range_max * 0.3
        self.threshold_lower = self.trace_range_max * 0.05
    ...
    def take_measurement(self,location):
        ...
        centre_voltages = self.block_centre_voltages[location[0]][ location[1]]

        scan = self.pygor.do2d(self.control_gates[0], centre_voltages[0] + self.block_size/2.0, centre_voltages[0] - self.block_size/2.0, self.block_size,
                        self.control_gates[1], centre_voltages[1] + self.block_size/2.0, centre_voltages[1] - self.block_size/2.0, self.block_size)

        small_window_measurements = scan.data[0]
        
        small_window_statistics = self.block_splitting(small_window_measurements)

        self.total_measurement[location[1]*self.block_size: location[1]*self.block_size + self.block_size,location[0]*self.block_size: location[0]*self.block_size + self.block_size] = small_window_measurements
    ...
    def random_point_measurement(self,location):
        ...
        i = 0
        while any(stats == 0):
            small_window_measurements = self.sample_random_pixels(small_window_measurements,centre_voltages)
            stats = self.block_splitting_statistics(small_window_measurements)
            self.log_stats_mean.append(stats[0])
            self.log_stats_std.append(stats[8])
            i+=1
        small_window_measurements = self.sample_random_pixels(small_window_measurements,centre_voltages)
        new_stats = self.block_splitting_statistics(small_window_measurements)

        while sum(abs((stats - new_stats)/stats)) > 0.010:
            i+=1
            #print(i)
            small_window_measurements = self.sample_random_pixels(small_window_measurements,centre_voltages)
            stats = new_stats
            ...
            self.log_stats_mean.append(new_stats[0])
            self.log_stats_std.append(new_stats[8])

        small_window_statistics = self.block_splitting_statistics(small_window_measurements)

        return small_window_measurements, small_window_statistics

    def sample_random_pixels(self,small_window_measurements,centre_voltages):

        x, y = random.randint(0, self.block_size - 1), random.randint(0, self.block_size - 1)

        gate_values = (centre_voltages[0] + math.floor(self.block_size / 2.0) - x,
                       centre_voltages[1] + math.floor(self.block_size / 2.0) - y)
        self.pygor.setvals(self.control_gates, gate_values)
        point_measurement = self.pygor.do0d()
        small_window_measurements[x, y] = point_measurement[0][0]
    ...
    def step(self, action):
        ...
        measurement, statistics = self.primary_measurement([id2, id1])
    ...
    def check_for_bias_triangle(self, ii, jj):
        ...
        if self.cnn_prediction[ii,jj] > 0.5:
            print("Classifier prediction ",self.cnn_prediction[ii,jj])
