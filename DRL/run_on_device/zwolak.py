...
def run_tuning(region,starting_indices, pygor_xmlip, MaxStep):
    ...
    for item in tqdm(starting_indices):
        ...
        zwolak = Nelder_Mead_tuning(region, params, pygor_mode, pygor_xmlip, MaxStep)
        res = zwolak.main()

        t_run = datetime.now() - t0
        times.append(t_run.total_seconds())
        outcome.append(zwolak.bias_triangle_found)
        parameters.append(zwolak.params_list)
        data_log.append(zwolak.data_store)
        steps.append(zwolak.evaluations)

        print('Run time ', t_run.total_seconds())

        print('Sucsess ', zwolak.bias_triangle_found)
