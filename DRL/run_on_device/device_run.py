...
def run(model,epsilon,gates,gate_numbers,default_gates,initial_gate_cu_cv_num_indices,pygor_xmlip = None, show_log = False, MaxStep=100, starting_position=None, random_pixel = True, savefilename = None):
    ...
    '''centre_voltages = env.block_centre_voltages[math.floor(env.allowed_n_blocks/2.0)][math.floor(env.allowed_n_blocks/2.0)]
    data = env.pygor.do2d(env.control_gates[0], centre_voltages[0] + env.allowed_n_blocks/2.0 * env.block_size, centre_voltages[0] - env.allowed_n_blocks/2.0 * env.block_size,21, env.control_gates[1], centre_voltages[1] + env.allowed_n_blocks/2.0 * env.block_size ,
                   centre_voltages[1] - env.allowed_n_blocks/2.0 * env.block_size, 21)
    
    data = data.data[0]
    
    np.save('fine_tuning/full_scan'+date_time, data)
    
    
    plt.imshow(data,extent=[env.block_centre_voltages[0][0][0],env.block_centre_voltages[-1][-1][0],env.block_centre_voltages[-1][-1][1],env.block_centre_voltages[0][0][1]])
    plt.colorbar()
    plt.title("Full scan")
    plt.show()'''
