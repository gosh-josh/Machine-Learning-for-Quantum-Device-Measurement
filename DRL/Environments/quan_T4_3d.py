...
class Quantum_T4_3D: # 3D
    ...
    def possible_actions_from_location(self,location):
        ...
        if self.isRepeat is True:
            ...
            if id1>0 and id2<(self.dim[1]-1) and id3>0 : #decrease d1, d2, d3
                possible_actions.append(6)
            if (id1<self.dim[0]-1) and  (id2>0) and (id3<self.dim[2]-1): #increase d1, d2, d3
                possible_actions.append(7)    
        else:
            if id1>0 and self.visit_map[id1-1,id2,id3]==0: #decrease d1
                possible_actions.append(0)
            if id1<self.dim[0]-1 and self.visit_map[id1+1,id2,id3]==0: #increase d1
                possible_actions.append(1)
            if id2>0 and self.visit_map[id1,id2-1,id3]==0: #decrease d2
                possible_actions.append(2)
            if id2<self.dim[1]-1 and self.visit_map[id1,id2+1,id3]==0: #increase d2
                possible_actions.append(3)
            if id3>0 and self.visit_map[id1,id2,id3-1]==0: #decrease d3
                possible_actions.append(4)
            if id3<self.dim[2]-1 and self.visit_map[id1,id2,id3+1]==0: #increase d3
                possible_actions.append(5)
            if id1>0 and id2<(self.dim[1]-1) and id3>0 and self.visit_map[id1-1,id2+1,id3-1]==0: #decrease d1, d2, d3
                possible_actions.append(6)
            if (id1<self.dim[0]-1) and  (id2>0) and (id3<self.dim[2]-1) and self.visit_map[id1+1,id2-1,id3+1]==0: #increase d1, d2, d3
                possible_actions.append(7)    
    ...
    def construct_block_given_starting_locations(self, starting_pixel_locs=[0,0,0], w1=5, w2=10,w3=10):
        ...
        for ii in range(nDim1):
            ...
            for jj in range(nDim2):
                ...
                image_smallpatch_data[ii][jj]=[0]*nDim3
                for kk in range(nDim3):
                    ...
                    #patch_data[ii][jj][kk]=[]
                    temp_patch=[]
                    image_smallpatch_data[ii][jj][kk]=[0]*len(pp_blocks)
                    for uu,mypp in enumerate(pp_blocks): # 27 items
                        ...
                        # image of the block
                        image_smallpatch_data[ii][jj][kk][uu]=np.copy(temp)
                        
                    patch_data[ii][jj][kk]=temp_patch
        
        return patch_data, [nDim1,nDim2,nDim3],range_data,image_smallpatch_data
    ...
    def __init__(self, starting_pixel_loc,isRepeat=True):
        ...
        self.sd_dist=[0.1,0.1]
        
        self.dd_dist=[0.03, 0.05]
        ...
        temp = pickle.load( open( '../data/T4_res_150_win_300_3d_elapse_87499.p', "rb" ) )

        offset=2.038457145963552e-10
        ...
        self.extended_image=np.pad(self.image, (10, 10), 'edge')


        #self.data,self.dim,self.range_data=self.load_image(path)
        
        # base on this location, construct the data
        self.data,self.dim,self.range_data,self.image_smallpatch_data=self.construct_block_given_starting_locations(starting_pixel_loc)
    ...
    def get_neightborMapIndividual(self,location):
        ...
        if id1==0 or id2==self.dim[1]-1 or id3==0:
            output.append(0)
        else:
            output.append(self.visit_map[id1-1,id2+1,id3-1]/norm_factor)

        if id1==self.dim[0]-1 or id2==0 or id3==self.dim[2]-1:
            output.append(0)
        else:
            output.append(self.visit_map[id1+1,id2-1,id3+1]/norm_factor)
    ...
    def step(self,action):
        ...
        if action==0:
            ...
            if self.current_pos[0]==0 or self.current_pos[1]==self.dim[1]-1 or self.current_pos[2]==0:
                print("cannot decrease all gates")
            else:
                self.current_pos[0]=self.current_pos[0]-1   
                self.current_pos[1]=self.current_pos[1]+1   
                self.current_pos[2]=self.current_pos[2]-1   
        elif action==7:
            if self.current_pos[0]==self.dim[0]-1 or self.current_pos[1]==0 or self.current_pos[2]==self.dim[2]-1:
                print("cannot increase all gates")
            else:
                self.current_pos[0]=self.current_pos[0]+1   
                self.current_pos[1]=self.current_pos[1]-1   
                self.current_pos[2]=self.current_pos[2]+1   
    ...
    def check_where_is_quantum_2d(self,ii,jj,kk):
        obs=np.reshape(self.data[ii][jj][kk],(3**len(self.dim),2)) # for predicting quantum, we use multiple smaller blocks
        countQ=0
        countB=0
        for uu in range(3**len(self.dim)): # 3**len(self.dim) = 9
            ...
            if isQuantum is True:
                ...
                myimg=self.image_smallpatch_data[ii][jj][kk][uu]
                count=0
                for oo in range(myimg.shape[0]):
                    
                    stdZero,stdOne=self.compute_entropy_block(myimg[oo,:,:])
                #print(ii,jj,uu,obs[uu],stdOne)
        ...
        if (countQ>=1 and countQ<=8) and countB>=19:
            #print(ii,jj,kk)
            return 1
        else:
            return 0
    ...
    def where_is_quantum(self):
        ...
        for ii in range(ndim1):
            for jj in range(ndim2):
                for kk in range(ndim3):
                    self.isquantum[ii,jj,kk]=self.check_where_is_quantum_2d(ii,jj,kk)
        ...
        for ii in range(ndim1):
            for jj in range(ndim2):
                for kk in range(ndim3):
                    ...
                    if (countQ==1 or countQ==2) and countB>=24:
